...
function cityStateQuery(city, state) {
    if (city && state && state !== "US")
        return `${city}, ${state}, USA`;
    if (state && state !== "US")
        return `${state}, USA`;
    return null;
}
function cityStateFromAddressPart(addressPart) {
    if (!addressPart)
        return null;
...
export async function geocodeOneQuery(query) {
 ...
 */
export async function geocodeAttraction(attraction) {
    const parsed = parseCityStateFromAddress(attraction.address);
    const city = attraction.city ?? parsed.city;
    const state = attraction.state && attraction.state !== "US" ? attraction.state : parsed.state;
    ...
    const queriesToTry = [primaryQuery, fallbackQuery, addressPartFallback].filter((q) => !!q && q.length > 0);
