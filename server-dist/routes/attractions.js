...
export async function attractionsRoutes(app) {
    ...
    app.get("/", async (request, reply) => {
        ...
        if (category?.trim())
            where.attractionCategories = { some: { category: { slug: category.trim() } } };
        ...
        if (sortBy === "rating") {
            ...
            const ratingStats = allIds.length > 0
                ? await prisma.checkIn.groupBy({
                    by: ["attractionId"],
                    _avg: { rating: true },
                    _count: { id: true },
            ...
            const items = attractions.map((a) => {
                ...
                const dist = hasUserLocation && a.latitude != null && a.longitude != null
        ...
        const orderBy = sortBy === "visitCount"
            ? { checkIns: { _count: sortOrder } }
            : { [sortBy]: sortOrder };
    ...
    app.get("/:id", async (request, reply) => {
        ...
        const likeCounts = checkInIds.length > 0
    ...
    app.get("/nearby/explore", async (request, reply) => {
        const q = querySchema.safeParse(request.query);
        if (!q.success) {
            return reply.status(400).send({ error: q.error.flatten() });
        }
