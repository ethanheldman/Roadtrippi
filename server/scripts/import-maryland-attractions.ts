/**
 * Import Maryland roadside attractions from Roadside America list.
 * Data in scripts/data/maryland-ra.txt, one "City: Name" per line.
 * Run from server: npx tsx scripts/import-maryland-attractions.ts
 */
...
async function main() {
  const dataPath = join(process.cwd(), "scripts", "data", "maryland-ra.txt");
  ...
  for (const { city, name } of entries) {
    ...
    const att = await prisma.attraction.create({
      data: {
        ...
        city,
        state: STATE,
        description: `Maryland roadside attraction in ${city}.`,
      },
    });
    if (catId) {
  ...
  console.log(
    `Maryland attractions: created ${created}, skipped (already exist) ${skipped}.`
  );
