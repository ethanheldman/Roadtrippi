/**
 * Import California roadside attractions from Roadside America list.
 * Data in scripts/data/california-ra.txt, one "City: Name" per line.
 * Run from server: npx tsx scripts/import-california-attractions.ts
 */
...
async function main() {
  const dataPath = join(process.cwd(), "scripts", "data", "california-ra.txt");
  ...
  for (const { city, name } of entries) {
    ...
    const att = await prisma.attraction.create({
      data: {
        ...
        city,
        state: STATE,
        description: `California roadside attraction in ${city}.`,
      },
    });
    if (catId) {
  ...
  console.log(
    `California attractions: created ${created}, skipped (already exist) ${skipped}.`
  );
