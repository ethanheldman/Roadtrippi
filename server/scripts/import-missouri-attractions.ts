/**
 * Import Missouri roadside attractions from Roadside America list.
 * Data in scripts/data/missouri-ra.txt, one "City: Name" per line.
 * Run from server: npx tsx scripts/import-missouri-attractions.ts
 */
...
async function main() {
  const dataPath = join(process.cwd(), "scripts", "data", "missouri-ra.txt");
  ...
  for (const { city, name } of entries) {
    ...
    const att = await prisma.attraction.create({
      data: {
        ...
        city,
        state: STATE,
        description: `Missouri roadside attraction in ${city}.`,
      },
    });
    if (catId) {
  ...
  console.log(
    `Missouri attractions: created ${created}, skipped (already exist) ${skipped}.`
  );
