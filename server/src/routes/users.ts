...
import { computeEarnedBadges, type BadgeContext } from "../lib/badges.js";
...
export async function usersRoutes(app: FastifyInstance) {
   ...
   */
  app.get<{ Params: { id: string } }>("/:id/badges", async (request, reply) => {
    const { id: targetId } = request.params;
    ...
    for (const c of checkIns) {
      if (c.attraction?.state) states.add(c.attraction.state.slice(0, 2).toUpperCase());
      for (const ac of c.attraction?.attractionCategories ?? []) {
        categories.add(ac.categoryId);
      }
      if (c.rating === 5) fiveStar++;
      if (c.review && c.review.trim().length > 0) reviewed++;
      const dayKey = c.visitDate.toISOString().slice(0, 10);
      perDay.set(dayKey, (perDay.get(dayKey) ?? 0) + 1);
      if (!earliest || c.visitDate < earliest) earliest = c.visitDate;
    }
