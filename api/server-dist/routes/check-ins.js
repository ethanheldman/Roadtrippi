...
export async function checkInsRoutes(app) {
    ...
    app.patch("/:id", { preHandler: auth }, async (request, reply) => {
        ...
        if (!checkInId)
            return reply.status(400).send({ error: "Check-in ID required" });
        ...
        if (!existing)
            return reply.status(404).send({ error: "Check-in not found" });
        if (existing.userId !== userId)
            return reply.status(403).send({ error: "Not allowed to update this check-in" });
        const data = {};
        if (body.data.rating !== undefined)
            data.rating = body.data.rating;
        if (body.data.review !== undefined)
            data.review = body.data.review ?? null;
        if (body.data.visitDate !== undefined)
            data.visitDate = new Date(body.data.visitDate);
