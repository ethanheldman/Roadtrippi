...
export async function listsRoutes(app) {
        ...
        if (!list.public) {
            try {
                ...
                if (userId !== list.userId)
                    return reply.status(403).send({ error: "Not allowed to view this list" });
            }
            catch {
                return reply.status(401).send({ error: "Unauthorized" });
            }
        }
        const ratingStats = list.listItems.length > 0
    ...
    /** Like a list */
    app.post("/:id/like", { preHandler: auth }, async (request, reply) => {
        const { userId } = await requireAuth(request, reply);
        ...
        if (!list)
            return reply.status(404).send({ error: "List not found" });
        if (!list.public)
            return reply.status(403).send({ error: "Cannot like a private list" });
    ...
    /** Unlike a list */
    app.delete("/:id/like", { preHandler: auth }, async (request, reply) => {
        const { userId } = await requireAuth(request, reply);
        const { id: listId } = request.params;
    ...
    app.post("/:id/comments", { preHandler: auth }, async (request, reply) => {
        ...
        if (!list)
            return reply.status(404).send({ error: "List not found" });
        if (!list.public)
            return reply.status(403).send({ error: "Cannot comment on a private list" });
    ...
    app.delete("/:id/comments/:commentId", { preHandler: auth }, async (request, reply) => {
        const { userId } = await requireAuth(request, reply);
    ...
    app.post("/:id/items", { preHandler: auth }, async (request, reply) => {
        const { userId } = await requireAuth(request, reply);
        const { id: listId } = request.params;
        ...
        if (!attraction)
            return reply.status(404).send({ error: "Attraction not found" });
    ...
    app.delete("/:id/items/:attractionId", { preHandler: auth }, async (request, reply) => {
        const { userId } = await requireAuth(request, reply);
