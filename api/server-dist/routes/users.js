...
export async function usersRoutes(app) {
    ...
    app.post("/me/avatar", { preHandler: auth }, async (request, reply) => {
    ...
    app.get("/me/social", { preHandler: auth }, async (request, reply) => {
        const { userId } = await requireAuth(request, reply);
    ...
    app.get("/me/followers", { preHandler: auth }, async (request, reply) => {
        const { userId } = await requireAuth(request, reply);
    ...
    app.get("/me/following", { preHandler: auth }, async (request, reply) => {
        const { userId } = await requireAuth(request, reply);
        const rows = await prisma.follow.findMany({
            where: { followerId: userId },
    ...
    app.get("/me/friends", { preHandler: auth }, async (request, reply) => {
        const { userId } = await requireAuth(request, reply);
        const following = await prisma.follow.findMany({
            where: { followerId: userId },
            select: { followingId: true },
    ...
    app.get("/me/feed", { preHandler: auth }, async (request, reply) => {
        const { userId } = await requireAuth(request, reply);
    ...
    app.get("/me/inbox", { preHandler: auth }, async (request, reply) => {
        const { userId } = await requireAuth(request, reply);
        ...
        const checkInById = Object.fromEntries(checkInsWithAttraction.map((c) => [c.id, c]));
        const attractionByCheckInId = Object.fromEntries(checkInsWithAttraction.map((c) => [c.id, c.attraction]));
    ...
    app.post("/:id/follow", { preHandler: auth }, async (request, reply) => {
        const { userId } = await requireAuth(request, reply);
        const { id: targetId } = request.params;
        if (targetId === userId)
            return reply.status(400).send({ error: "You can't follow yourself. Use another account to follow this user." });
    ...
    app.delete("/:id/follow", { preHandler: auth }, async (request, reply) => {
        const { userId } = await requireAuth(request, reply);
        const { id: targetId } = request.params;
    ...
    app.get("/:id/following", { preHandler: auth }, async (request, reply) => {
        const { userId } = await requireAuth(request, reply);
        const { id: targetId } = request.params;
    ...
    app.get("/:id/following/list", async (request, reply) => {
    ...
    app.get("/:id/followers/list", async (request, reply) => {
        const { id } = request.params;
        const exists = await prisma.user.findUnique({ where: { id }, select: { id: true } });
        if (!exists)
            return reply.status(404).send({ error: "User not found" });
